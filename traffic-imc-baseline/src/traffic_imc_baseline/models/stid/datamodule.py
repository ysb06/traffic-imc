...
from .dataset import STIDDataset

STIDSample = Tuple[torch.Tensor, torch.Tensor, torch.Tensor]
STIDTrainBatch = Tuple[torch.Tensor, torch.Tensor]
STIDTestBatch = Tuple[torch.Tensor, torch.Tensor, torch.Tensor]


def collate_stid_train(batch: list[STIDSample]) -> STIDTrainBatch:
...
def collate_stid_test(batch: list[STIDSample]) -> STIDTestBatch:
...
class STIDDataModule(L.LightningDataModule):
    """PyTorch Lightning DataModule for STID."""
    ...
    def __init__(
        ...
        collate_fn: Callable[[list[STIDSample]], STIDTrainBatch] = collate_stid_train,
        test_collate_fn: Callable[[list[STIDSample]], STIDTestBatch] = collate_stid_test,
    ) -> None:
        super().__init__()
        ...
        self.train_dataset: Optional[STIDDataset] = None
        self.val_dataset: Optional[STIDDataset] = None
        self.test_dataset: Optional[STIDDataset] = None
    ...
    def _apply_scaling(self, *datasets: STIDDataset) -> None:
        if self._scaler is None:
    ...
    def setup(
        ...
        self.train_dataset = STIDDataset(
            train_df,
            in_steps=self.in_steps,
            out_steps=self.out_steps,
            missing_mask=None,
        )
        self.val_dataset = STIDDataset(
            val_df,
            in_steps=self.in_steps,
            out_steps=self.out_steps,
            missing_mask=None,
        )
        self.test_dataset = STIDDataset(
            test_df,
            in_steps=self.in_steps,
            out_steps=self.out_steps,
