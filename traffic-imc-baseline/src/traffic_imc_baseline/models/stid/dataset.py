...
class STIDDataset(Dataset):
    """PyTorch Dataset for STID.

    Each sample returns exactly:
    - x: (in_steps, n_vertex, 3) where features are [traffic, tod, dow]
    - y: (out_steps, n_vertex, 1)
    - y_is_missing: (out_steps, n_vertex)

    The temporal features follow the original STID convention:
    - tod is normalized to [0, 1)
    - dow is normalized as dayofweek / 7
    ...
    def __init__(
        ...
        if len(self.valid_indices) == 0:
            raise ValueError(
                "No valid STID samples generated. "
    ...
    def _extract_tod(self, index: pd.DatetimeIndex) -> np.ndarray:
        ...
        return ((minutes * 60 + seconds) / (24 * 60 * 60)).values.astype(np.float32)

    def _extract_dow(self, index: pd.DatetimeIndex) -> np.ndarray:
        return (index.dayofweek / 7).values.astype(np.float32)
